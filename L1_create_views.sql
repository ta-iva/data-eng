...
WHERE id_status IS NOT NULL
  ...
  -- na lekci doporučovaná operace výše hrozí potenciální ztrátou validních hodnot 
...
SELECT
  ...
  -- pokud je hodnota load_date důležitá, třeba zajistit, aby byla vygenerovaná ve správné časové zóně ještě před importem dat
  , load_date
...
FROM `united-skyline-463312-b6.L0_crm.product_purchase` pp
LEFT JOIN `united-skyline-463312-b6.L1.L1_product` pr
  ON pp.id_package_template = pr.product_id
LEFT JOIN `united-skyline-463312-b6.L1.L1_status` s
